...
function loadCards() {
    fetch(`/api/opinions?offer_pro_id=${user.account_id}&limit=5&offset=${offset}`)
            ...
            for (let opinion of opinions) {
                if (user) {
                    if (opinion.user.account_id !== user.account_id) {
                        ...
                        opinionsContainer.insertBefore(createOpinionCard(opinion), loaderSection);
                    }
                } else {
                    opinionsContainer.insertBefore(createOpinionCard(opinion), loaderSection);
                }
        ...
        <header class="not-read">
            <div>
                ...
                <div class="flex flex-col gap-1 justify-center">
                    ...
                    <p class="text-sm">Avis sur <a href="/offres/${opinion.offer_id}" class="underline">${opinion.offer.title}</a></p>
                </div>
            </div>
...
searchInput.addEventListener('input', debounce(() => {
    ...
    fetch(`/api/opinions?offer_pro_id=${user.account_id}&limit=5&offset=${offset}&q=${value}`)
        .then(response => response.json())
        .then(opinions => {
            if (opinions.length < 5) {
                loaderSection.classList.add('hidden');
