...
export function createOpinionCard(opinion, dashboard = false, blacklisted = false) {
    ...
    card.innerHTML = `
        <!-- Header -->
            ...
            `<button class="blacklist-card" data-dialog-trigger="blacklist-dialog" title="blacklist">
                            ...
                            <div class="flex px-4 py-2 justify-between items-center gap-4 ${opinion.offer.token_number > 2 ? 'color-green' : opinion.offer.token_number >= 1 ? 'color-yellow' : 'color-red'}">
                                <span class="token-tooltip">
                                    ${opinion.offer.token_number} jetons
                                    <span class="tooltip-text font-normal">
                                        Nombre de vetos disponibles 
                                    </span>
                                </span>
        ...
        <!-- Stars -->
        ${dashboard ? `<div class="opinion-card-stars mb-2">
            <p class="text-sm text-gray-4">A noté</p>
            ${stars.outerHTML}
        ...
        <!-- Pop-up Blacklist -->
        <div class="dialog-container-blacklist hide" data-dialog-name="">
            <div class="dialog w-[70%]">
                <header class="dialog-header">
                ...
                <div class="dialog-content">
                    ...
                    <div class="popup-class on-same-line grid lg:grid-cols-2 sm:grid-cols-1 gap-4 mt-8">
                        ...
                        <button type="button" class="button cancel-button-blacklist">Annuler</button>
                    </div>
                </div>
            </div>
        </div>
`;
    ...
    let blacklistSvg = null;

    if(opinion.offer.type==="premium" && opinion.blacklistage_possible===true){
        blacklistButton = card.querySelector(".blacklist-card");
    }

    if(blacklistButton){
        blacklistSvg = blacklistButton.querySelector('svg');
    }

    if (blacklistButton && blacklistSvg){
        blacklistButton.addEventListener("click", () => {
            const dialog = card.querySelector('.dialog-container-blacklist');
            // console.log(dialog);
            dialog.classList.remove('hide');
        });

        let cancelButtonBlacklist = card.querySelector('.cancel-button-blacklist');
        cancelButtonBlacklist.addEventListener('click', () => {
            const dialog = card.querySelector('.dialog-container-blacklist');
            dialog.classList.add('hide');
        });
        ...
        confirmButtonBlacklist.addEventListener('click', () => {
            if(opinion.blacklistage_possible){
                let opinionId = opinion.id;
                fetch(`/api/opinions/${opinionId}/blacklist`, {
                    method: 'GET',
                }).then(response => response.json())
