class Carousel {
    ...
    setStyle() {
        ...
        this.container.style.width = (ratio * 101.5) + "%"
        this.items.forEach((item, index) => {
            item.style.width = `calc(${(100 / this.slidesVisible) / ratio}% - ${this.options.gap}rem)`;
            item.style.marginRight = this.options.gap + "rem"
        })
    }
